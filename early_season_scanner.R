...
scan_early_season <- function(model_path = MODEL_PATH,
  ...
  results <- combined %>%
    ...
    mutate(
      ...
      # Projected HR: predicted HR/BBE rate × prior-season BBE (as season-length proxy)
      projected_hr_rate = pmax(hr_per_bbe_y1 + predicted_delta_hr_bbe, 0),
      projected_hr = round(projected_hr_rate * bbe_y1),
      # Confidence based on current season BBE count
    ...
    select(
      ...
      predicted_delta_hr_bbe, projected_hr, breakout_score, confidence,
      any_of("age"),
      any_of(c(
  ...
  report_lines <- c(
    ...
    "",
    "| Rank | Player | Proj. HRs | Predicted HR/BBE Change | Breakout Score | Confidence | Last Year HR/BBE | BBE |",
    "|------|--------|-----------|------------------------|----------------|------------|-----------------|-----|"
  )
  ...
  report_lines <- c(report_lines,
    ...
    "",
    "- **Proj. HRs**: Projected full-season HR count (predicted HR/BBE rate × prior-season BBE)",
    "- **Predicted HR/BBE Change**: Model's predicted change in HR per batted ball event vs last season (e.g. +2.0% means model expects 2 more HRs per 100 BBE)",
    "- **Last Year HR/BBE**: Prior full-season HR rate — the stable baseline used for ceiling and projection calculations",
    "- **Breakout Score**: Composite score (45% model, 15% HR/BBE ceiling, 12% LA direction, 12% discipline, 8% flyball, 8% age)",
    "- **Confidence**: Based on current season BBE count (High: ≥100, Moderate: ≥50, Low: <50)",
    ""
  )
