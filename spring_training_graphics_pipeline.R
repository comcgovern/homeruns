# =============================================================================
# spring_training_graphics_pipeline.R
#
# Pipeline: spring_training_hr_prediction CSV → computed gainers/losers →
#           self-contained HTML (React + Babel CDN) → Playwright → PNG
#
# Finds the most recent spring_training_hr_prediction_*.csv in output/ automatically.
#
...
# ── 1. Find most recent spring training prediction CSV ────────────────────────

csv_files <- list.files(OUTPUT_DIR,
                        pattern  = "^spring_training_hr_prediction_.*\\.csv$",
                        full.names = TRUE)

if (length(csv_files) == 0) {
  stop("No spring_training_hr_prediction_*.csv files found in ", OUTPUT_DIR, "/")
}
...
# Expects a filename like: spring_training_hr_prediction_YYYY-MM-DD.csv

date_match <- regmatches(
  basename(INPUT_CSV),
  regexpr("(\\d{4}-\\d{2}-\\d{2})", basename(INPUT_CSV))
)
if (length(date_match) == 1) {
  scan_label <- format(as.Date(date_match), "%b %d, %Y")
} else {
  scan_label <- "Spring Training Scan"
}
      ...
      <div className="card" id="hr-card">
        <div className="card-header" style={{ borderBottom: `3px solid ${accent}` }}>
          <div className="header-inner">
            <div>
              <div className="card-title">{isGainers ? "ST HR Gainers" : "ST HR Losers"}</div>
              <div className="card-sub">Spring Training Scan \u00b7 {SCAN_LABEL}</div>
            </div>
...
html_path <- normalizePath(
  file.path(OUTPUT_DIR, "st_hr_model_graphic.html"),
  mustWork = FALSE
)
...
gainers_png <- normalizePath(file.path(OUTPUT_DIR, "st_hr_gainers.png"), mustWork = FALSE)
losers_png  <- normalizePath(file.path(OUTPUT_DIR, "st_hr_losers.png"),  mustWork = FALSE)
...
pw_path <- file.path(OUTPUT_DIR, "st_screenshot.js")
